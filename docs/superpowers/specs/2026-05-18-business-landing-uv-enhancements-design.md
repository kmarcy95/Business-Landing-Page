...
[AMA®] [ChFM®] [GAFM Advisor] [MA Economics — UTSA] [BS Finance — Evansville]

(Confirm with user during implementation whether the MA is completed or still in progress — resume said "currently enrolled" as of late 2023.)
```
...
User must place two image files before deploy:

| Path | Source | Treatment |
|------|--------|-----------|
| `images/headshot-hero.jpg` | Image 4 (composed, navy blazer, dark studio) | 1024×1024 (1:1) as supplied is fine, JPEG quality ~82, ≤200 KB |
| `images/headshot-about.jpg` | Image 3 (smiling, navy blazer, dark studio) | 1024×1024 (1:1) as supplied is fine, JPEG quality ~82, ≤120 KB |

Both source images are 2048×2048; user can downscale to 1024 and compress to JPEG before drop-in. No cropping required — CSS handles framing via `object-fit: cover` and aspect-ratio.
...
- `images/headshot-hero.jpg` — new (user-provided)
- `images/headshot-about.jpg` — new (user-provided)
